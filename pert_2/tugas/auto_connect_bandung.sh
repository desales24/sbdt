...
echo "Memulai sistem monitoring dan auto-connect ke Node Bandung..."

while true; do
    # Mengecek apakah database merespon (ping sederhana)
    if docker exec node_bandung psql -U postgres -d penjualan -c "SELECT 1;" > /dev/null 2>&1; then
        echo "======================================================"
        echo "[STATUS: HIDUP] Berhasil terhubung ke Node Bandung!"
        echo "Membuka terminal database..."
        echo "======================================================"
        
        # Masuk ke terminal interaktif
        docker exec -it node_bandung psql -U postgres -d penjualan
        EXIT_CODE=$?
        ...
        echo "======================================================"
        echo "[PERINGATAN] Server Bandung sedang MATI atau tidak dapat diakses!"
